...
#pragma clang diagnostic pop
	...
	void execute_decision_set(std::vector<std::pair<nations::country_tag, events::decision_tag>>const& decision_set, world_state& ws) {
		for(auto& p : decision_set) {
			...
			triggers::execute_effect(ws.s.trigger_m.effect_data.data() + to_index(ws.s.event_m.decision_container[p.second].effect), ws, p.first, p.first, nullptr, nullptr, gen);
			gen.advance_n<8>();
		}
	}
}
