...
namespace economy {
	...
	public:
		void button_function(ui::simple_button<close_button_s>&, world_state&);
	};
...
#include "good_filter_item_window.h"
	...
	*/
	...
	using state_pop_display_window = ui::gui_window<
		...
		CT_STRING("pop_amount"), ui::display_text<state_pop_display_amount, 4>,
...
#include "economy_windows_b.h"
	...
	*/
	...
	using state_window = ui::gui_window<
		...
		CT_STRING("state_name"), ui::display_text<state_name, 4>,
		CT_STRING("factory_count"), ui::display_text<state_factory_count, 4>,
	...
	*/
	...
	using factory_worker_item = ui::gui_window<
		...
		CT_STRING("pop_amount"), ui::display_text<empty_text_box>,
		CT_STRING("pop_amount_2"), ui::display_text<factory_worker_item_amount>,
...
#include "economy_windows.h"
	...
	void state_window_base::on_create(W & win, world_state & ws) {
		...
		workers_a.associated_object->position.y = -2i16;
		...
		workers_b.associated_object->position.y = -2i16;
	...
	void investment_country_item_self_investment_text::windowed_update(window_type & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(auto player = ws.w.local_player_nation; bool(player) && is_valid_index(win.tag)) {
			ui::add_text(ui::xy_pair{ 0,0 }, text_data::currency{ nations::get_foreign_investment(ws, player, win.tag) },
				fmt, ws, box, lm);
		}
	}
	template<int32_t index>
	...
	void gp_investment_text<index>::windowed_update(window_type & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		...
		if(begin(r) + index < end(r)) {
			if(auto id = r.first[index]; ws.w.nation_s.nations.is_valid_index(id)) {
				ui::add_text(ui::xy_pair{ 0,0 }, text_data::currency{ nations::get_foreign_investment(ws, id, win.tag) },
					fmt, ws, box, lm);
			}
		}
	}
	...
	void investment_country_item_military_rank::windowed_update(window_type & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(win.tag)) {
			ui::add_text(ui::xy_pair{ 0,0 }, text_data::integer{ ws.w.nation_s.nations.get<nation::military_rank>(win.tag) },
				fmt, ws, box, lm);
		}
	}
	template<typename window_type>
	void investment_country_item_industrial_rank::windowed_update(window_type & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(win.tag)) {
			ui::add_text(ui::xy_pair{ 0,0 }, text_data::integer{ ws.w.nation_s.nations.get<nation::industrial_rank>(win.tag) },
				fmt, ws, box, lm);
		}
	}
	template<typename window_type>
	void investment_country_item_prestige_rank::windowed_update(window_type & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(win.tag)) {
			ui::add_text(ui::xy_pair{ 0,0 }, text_data::integer{ ws.w.nation_s.nations.get<nation::prestige_rank>(win.tag) },
				fmt, ws, box, lm);
		}
	}
	template<typename window_type>
	void investment_country_item_overall_rank::windowed_update(window_type & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(win.tag)) {
			ui::add_text(ui::xy_pair{ 0,0 }, text_data::integer{ ws.w.nation_s.nations.get<nation::overall_rank>(win.tag) },
				fmt, ws, box, lm);
		}
	}
	template<typename window_type>
	void investment_country_item_opinion_type::windowed_update(window_type & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(auto player = ws.w.local_player_nation; bool(player) && is_valid_index(win.tag)) {
			ui::add_text(ui::xy_pair{ 0,0 },
				nations::influence_level_to_text(ws, nations::get_influence_level(ws, player, win.tag)), fmt, ws, box, lm);
		}
	}
	template<typename window_type>
	void investment_country_item_relations_value::windowed_update(window_type & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(auto player = ws.w.local_player_nation; bool(player) && is_valid_index(win.tag)) {
			ui::add_text(ui::xy_pair{ 0,0 }, text_data::integer{ nations::get_relationship(ws, player, win.tag) },
				fmt, ws, box, lm);
		}
	}
	template<typename window_type>
	void investment_country_item_factories_count::windowed_update(window_type & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(win.tag)) {
			...
			ui::add_text(ui::xy_pair{ 0,0 }, text_data::integer{ total },
				fmt, ws, box, lm);
		}
	}
	template<typename lb_type>
	void investment_country_lb::populate_list(lb_type & lb, world_state & ws) {
		...
		switch(ws.w.production_w.sort_type) {
			...
			case country_sort::gp_one:
				if(auto r = get_range(ws.w.nation_s.nations_arrays, ws.w.nation_s.nations_by_rank); begin(r) + 0 < end(r)) {
			...
			case country_sort::gp_two:
				if(auto r = get_range(ws.w.nation_s.nations_arrays, ws.w.nation_s.nations_by_rank); begin(r) + 1 < end(r)) {
			...
			case country_sort::gp_three:
				if(auto r = get_range(ws.w.nation_s.nations_arrays, ws.w.nation_s.nations_by_rank); begin(r) + 2 < end(r)) {
			...
			case country_sort::gp_four:
				if(auto r = get_range(ws.w.nation_s.nations_arrays, ws.w.nation_s.nations_by_rank); begin(r) + 3 < end(r)) {
			...
			case country_sort::gp_five:
				if(auto r = get_range(ws.w.nation_s.nations_arrays, ws.w.nation_s.nations_by_rank); begin(r) + 4 < end(r)) {
			...
			case country_sort::gp_six:
				if(auto r = get_range(ws.w.nation_s.nations_arrays, ws.w.nation_s.nations_by_rank); begin(r) + 5 < end(r)) {
			...
			case country_sort::gp_seven:
				if(auto r = get_range(ws.w.nation_s.nations_arrays, ws.w.nation_s.nations_by_rank); begin(r) + 6 < end(r)) {
			...
			case country_sort::gp_eight:
				if(auto r = get_range(ws.w.nation_s.nations_arrays, ws.w.nation_s.nations_by_rank); begin(r) + 7 < end(r)) {
	...
	void gp_subwindow_investment_value::windowed_update(W & w, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		...
		if(begin(r) + w.nth_nation < end(r)) {
			if(auto id = r.first[w.nth_nation]; ws.w.nation_s.nations.is_valid_index(id)) {
				ui::add_text(ui::xy_pair{ 0,0 }, text_data::currency{ nations::get_foreign_investment(ws, id, ws.w.production_w.foreign_investment_nation) },
					fmt, ws, box, lm);
			}
		}
	}
	...
	void state_pop_display_amount::windowed_update(W & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(win.type) && is_valid_index(win.in_state)) {
			if(ws.s.population_m.capitalist == win.type) {
				...
				ui::add_text(ui::xy_pair{ 0,0 }, text_data::percent{ percent },
					fmt, ws, box, lm);
			} else {
				...
				ui::add_text(ui::xy_pair{ 0,0 }, text_data::exact_integer{ unemployed_pop },
					fmt, ws, box, lm);
			}
		}
	}
	...
	template<typename W>
	void state_name_s::windowed_update(W & w, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(w.tag)) {
			ui::add_text(ui::xy_pair{ 0,0 }, ws.w.nation_s.states.get<state::name>(w.tag), fmt,
				ws, box, lm);
		}
	}
	...
	void state_factory_count::windowed_update(W & w, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(w.tag)) {
			ui::add_text(ui::xy_pair{ 0,0 }, text_data::exact_integer{ economy::count_factories_in_state(ws, w.tag) },
				fmt, ws, box, lm);
		}
	}
	...
	void state_build_factory_button::windowed_update(ui::simple_button<state_build_factory_button>& self, window_type & win, world_state & ws) {
		tag = win.tag;
		self.set_enabled(can_build_factory_in_state(ws, ws.w.local_player_nation, tag));
	}
	...
	void state_average_infrastructure::windowed_update(W & w, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(w.tag)) {
			ui::add_text(ui::xy_pair{ 0,0 }, text_data::fp_one_place{ economy::average_railroad_level(ws, w.tag) },
				fmt, ws, box, lm);
		}
	}
	...
	void factory_level_text::windowed_update(W & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(win.index >= 0 && is_valid_index(win.location)) {
			economy::factory_instance& f = ws.w.nation_s.states.get<state::factories>(win.location)[win.index];
			if(f.level > 0) {
				ui::add_text(ui::xy_pair{ 0,0 }, text_data::exact_integer{ f.level },
					fmt, ws, box, lm);
			}
		}
	}
		...
		if(win.index >= 0 && is_valid_index(win.location)) {
			...
			if(factory_is_open(f)) {
				if(auto state_cap = nations::get_state_capital(ws, win.location); is_valid_index(state_cap)) {
					...
					ui::add_text(ui::xy_pair{ 0,0 }, text_data::currency{ profit },
						blk, ws, box, lm);
				}
			}
		}
	...
	template<typename W>
	void factory_closed_text_s::windowed_update(W & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(win.index >= 0 && is_valid_index(win.location)) {
			economy::factory_instance& f = ws.w.nation_s.states.get<state::factories>(win.location)[win.index];
			if(factory_is_closed(f)) {
				ui::add_text(ui::xy_pair{ 0,0 }, ws.s.fixed_ui_text[scenario::fixed_ui::closed], fmt, ws, box, lm);
			}
		}
	}
	...
	void goods_category_name::windowed_update(W & w, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(w.category)) {
			ui::add_text(ui::xy_pair{ 0,0 }, ws.s.economy_m.good_type_names[w.category], fmt, ws, box, lm);
		}
	}
	...
	void project_state_name::windowed_update(W & w, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(w.location)) {
			ui::add_text(ui::xy_pair{ 0,0 }, ws.w.nation_s.states.get<state::name>(w.location), fmt, ws, box, lm);
		}
	}
	...
	void project_investor_amount::windowed_update(W & w, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(w.location)) {
			ui::add_text(ui::xy_pair{ 0,0 }, text_data::currency{ ws.w.nation_s.states.get<state::project>(w.location).funds },
				fmt, ws, box, lm);
		}
	}

	template<typename W>
	void project_cost_s::windowed_update(W & w, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(w.location)) {
			...
			ui::add_text(ui::xy_pair{ 0,0 }, text_data::currency{ cost },
				fmt, ws, box, lm);
		}
	}

	template<typename W>
	void project_name_s::windowed_update(W & w, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(is_valid_index(w.location)) {
			...
			if(ws.w.nation_s.states.get<state::project>(w.location).type == economy::pop_project_type::railroad) {
				if(auto p = ws.w.nation_s.states.get<state::project>(w.location).location; is_valid_index(p)) {
					...
					if(ws.w.province_s.province_state_container.get<province_state::railroad_level>(p) == 0) 
						cursor = ui::add_text(cursor, ws.s.fixed_ui_text[scenario::fixed_ui::build], fmt, ws, box, lm);
					else 
						cursor = ui::add_text(cursor, ws.s.fixed_ui_text[scenario::fixed_ui::expand], fmt, ws, box, lm);
					cursor = ui::advance_cursor_by_space(cursor, ws.s.gui_m, fmt);
					ui::add_text(cursor, ws.s.fixed_ui_text[scenario::fixed_ui::railroad], fmt, ws, box, lm);
				}
				...
				if(auto ft = ws.w.nation_s.states.get<state::project>(w.location).factory_type; is_valid_index(ft)) {
					...
					if(ftype == economy::factory_project_type::expand)
						...
						cursor = ui::add_text(cursor, ws.s.fixed_ui_text[scenario::fixed_ui::reopen], fmt, ws, box, lm);
					else if(ftype == economy::factory_project_type::open)
						cursor = ui::add_text(cursor, ws.s.fixed_ui_text[scenario::fixed_ui::build], fmt, ws, box, lm);
					cursor = ui::advance_cursor_by_space(cursor, ws.s.gui_m, fmt);
					ui::add_text(cursor, ws.s.economy_m.factory_types[ft].name, fmt, ws, box, lm);
				}
			}
		}
	}
	...
	template<typename window_type>
	void factory_worker_item_icon::windowed_update(ui::dynamic_icon<factory_worker_item_icon>& self, window_type & win, world_state & ws) {
		tag = win.value.type;
		if(is_valid_index(tag))
			self.set_frame(ws.w.gui_m, ws.s.population_m.pop_types[tag].sprite - 1ui32);
	}

	template<typename W>
	void factory_worker_item_amount::windowed_update(W & w, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		ui::add_text(ui::xy_pair{ 0,0 }, text_data::integer{ w.value.amount }, fmt, ws, box, lm);
	}

	template<typename window_type>
	void factory_construction_cost_item_icon::windowed_update(ui::dynamic_icon<factory_construction_cost_item_icon>& self, window_type & win, world_state & ws) {
		if(tag = win.value.type; tag)
			self.set_frame(ws.w.gui_m, ws.s.economy_m.goods[tag].icon);
	}

	template<typename W>
	void factory_construction_cost_item_cost::windowed_update(W & w, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		auto const amount = w.value.amount;
		auto const s = ws.w.build_factory_w.in_state;
		if(auto const type = w.value.type; type && s) {
			auto const cost = amount * economy::state_current_prices(ws, s)[type];
			ui::add_text(ui::xy_pair{ 0,0 }, text_data::currency{ cost }, fmt, ws, box, lm);
		}
	}

	template<typename W>
	void factory_construction_cost_item_stockpile_amount::windowed_update(W & w, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
	}

	template<typename window_type>
	void factory_item_icon::windowed_update(ui::dynamic_icon<factory_item_icon>& self, window_type & win, world_state & ws) {
		auto const type = win.value;
		if(is_valid_index(type))
			self.set_frame(ws.w.gui_m, ws.s.economy_m.goods[type].icon);
	}

	template<typename W>
	void factory_item_name::windowed_update(W & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		auto const type = win.value;
		auto const s = ws.w.build_factory_w.in_state;
		if(auto const f_type = good_to_factory_type(ws, type); f_type && s) {
			ui::text_format lfmt{ factory_type_valid_in_state(ws, s, f_type) ? ui::text_color::black : ui::text_color::white,
				fmt.font_handle, fmt.font_size };
			ui::add_text(ui::xy_pair{ 0,0 }, ws.s.economy_m.factory_types[f_type].name, lfmt, ws, box, lm);
		}
	}

	template<typename W>
	void factory_item_cost::windowed_update(W & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		auto const type = win.value;
		auto const s = ws.w.build_factory_w.in_state;
		if(auto const f_type = good_to_factory_type(ws, type); f_type && s) {
	...
	template<typename W>
	void factory_item_time::windowed_update(W & win, ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		auto const type = win.value;
		if(auto const f_type = good_to_factory_type(ws, type); f_type) {
			ui::xy_pair cursor{ 0,0 };
			cursor = ui::add_text(cursor, text_data::integer{ ws.s.economy_m.factory_types[f_type].building_time }, fmt, ws, box, lm);
			cursor = ui::advance_cursor_by_space(cursor, ws.s.gui_m, fmt);
			cursor = ui::add_text(cursor, scenario::fixed_ui::tx_day, fmt, ws, box, lm);
		}
	}

	template<typename W>
	void build_factory_window_base::on_create(W & w, world_state &) {
		associated_object->size = ui::xy_pair{ 940i16, 496i16 };
		ui::hide(*associated_object);
	}

	template<int32_t number>
	void bf_input_amount<number>::update(ui::tagged_gui_object box, ui::text_box_line_manager & lm, ui::text_format & fmt, world_state & ws) {
		if(auto const f_type = good_to_factory_type(ws, ws.w.build_factory_w.factory_type); f_type) {
			int32_t j = 0;
			float amount = 0.0f;
			ws.s.economy_m.for_each_good([&j, &amount, &ws, f_type](goods_tag g) {
				if(ws.s.economy_m.factory_input_goods.get(f_type, g) > 0.0f) {
					if(j == number) {
						amount = ws.s.economy_m.factory_input_goods.get(f_type, g);
					}
					++j;
				}
			});
			...
			if(amount != 0.0f) {
				ui::add_text(ui::xy_pair{ 0,0 }, text_data::fp_two_places{ amount }, fmt, ws, box, lm);
			}
		}
		...
		if(auto const f_type = good_to_factory_type(ws, ws.w.build_factory_w.factory_type); f_type) {
			...
			for(uint32_t i = 0; i < std::extent_v<decltype(f_type_obj.factory_workers.workers)>; ++i) {
				if(is_valid_index(f_type_obj.factory_workers.workers[i].type)) {
					auto state_pop = 
						ws.w.nation_s.state_demographics.get(ws.w.build_factory_w.in_state, population::to_demo_tag(ws, f_type_obj.factory_workers.workers[i].type));
					potential_list.push_back(
						factory_worker_value{ state_pop, f_type_obj.factory_workers.workers[i].type }
					);
				}
			}
	...
	void factory_item_bg::windowed_update(ui::simple_button<factory_item_bg>& self, window_type& win, world_state& ws) {
		tag = win.value;
		if(ws.w.build_factory_w.factory_type != tag)
			self.set_frame(ws.w.gui_m, 0);
		else
