...
template<typename BASE, typename ELEMENT, int32_t left_expand>
void ui::display_listbox<BASE, ELEMENT, left_expand>::update_data(gui_object_tag, gui_static& sm, gui_manager& manager, world_state& w) {
	clear_items(manager);
	BASE::populate_list(*this, sm, manager, w);
	update_scroll_position(manager);
}

template<typename BASE, typename ELEMENT, int32_t left_expand>
template<typename window_type>
void ui::display_listbox<BASE, ELEMENT, left_expand>::windowed_update(window_type& w, gui_static& sm, gui_manager& m, world_state& s) {
	if constexpr(ui::detail::has_windowed_update<BASE, display_listbox<BASE, ELEMENT, left_expand>&, window_type&, gui_static&, gui_manager&, world_state&>) {
		clear_items(m);
		BASE::windowed_update(*this, w, sm, m, s);
		BASE::populate_list(*this, sm, m, w);
...
void ui::display_listbox<BASE, ELEMENT, left_expand>::add_item(gui_static& static_manager, gui_manager& manager, PARAMS&& ... params) {
	contents.emplace_back(std::forward<PARAMS>(params)...);
	auto& n = contents.back();

	create_static_element(static_manager, manager, element_def_tag, tagged_gui_object{ *_content_frame, _content_frame_tag }, n);
...
template<typename BASE, typename ELEMENT, int32_t left_expand>
void ui::display_listbox<BASE, ELEMENT, left_expand>::create_sub_elements(tagged_gui_object self, gui_static& static_manager, gui_manager& manager) {
	set_element_definition(static_manager);
	...
	ui::create_static_fixed_sz_scrollbar(
		static_manager,
		manager,
		static_manager.ui_definitions.standardlistbox_slider,
		self,
...
ui::tagged_gui_object ui::create_static_element(gui_static& static_manager, gui_manager& manager, listbox_tag handle, tagged_gui_object parent, ui::display_listbox<B, ELEMENT, left_expand>& b) {
	const ui::listbox_def& definition = static_manager.ui_definitions.listboxes[handle];
	...
	if (is_valid_index(definition.background_handle)) {
		ui::detail::instantiate_graphical_object(static_manager, manager, new_obj, definition.background_handle);
	}

	new_obj.object.position.x -= static_cast<int16_t>(left_expand);
	new_obj.object.size.x += static_cast<int16_t>(left_expand + 16);

	b.create_sub_elements(new_obj, static_manager, manager);
...
void ui::overlap_box<BASE, tag_type, ELEMENT, vertical_extension>::set_self_information(gui_static& sm, gui_manager& m, gui_object_tag s, int32_t sp, text_data::alignment a) {
	const auto t = BASE::element_tag(sm);
	element_def_tag = t;
	subelement_width = sm.ui_definitions.get(t).size.x;
...
template<typename BASE, typename tag_type, typename ELEMENT, int32_t vertical_extension>
template<typename ... PARAMS>
void ui::overlap_box<BASE, tag_type, ELEMENT, vertical_extension>::add_item(gui_static& static_manager, gui_manager& manager, PARAMS&& ... params) {
	if (is_valid_index(self) && is_valid_index(element_def_tag)) {
		contents.emplace_back(std::forward<PARAMS>(params)...);
		auto& n = contents.back();
		ui::create_static_element(static_manager, manager, element_def_tag, tagged_gui_object{ *associated_object, self }, n);
...
template<typename BASE, typename tag_type, typename ELEMENT, int32_t vertical_extension>
template<typename window_type>
void ui::overlap_box<BASE, tag_type, ELEMENT, vertical_extension>::windowed_update(window_type& w, gui_static& sm, gui_manager& m, world_state& s) {
	if constexpr(ui::detail::has_windowed_update<BASE, overlap_box<BASE, tag_type, ELEMENT, vertical_extension>&, window_type&, gui_static&, gui_manager&, world_state&>) {
...
template<typename BASE, typename tag_type, typename ELEMENT, int32_t vertical_extension>
void ui::overlap_box<BASE, tag_type, ELEMENT, vertical_extension>::update_data(gui_object_tag, gui_static& sm, gui_manager& manager, world_state& s) {
	if (is_valid_index(element_def_tag)) {
		clear_items(manager);
		BASE::populate_list(*this, sm, manager, s);
		update_item_positions(manager);
	}
}

template<typename B, typename tag_type, typename ELEMENT, int32_t vertical_extension>
ui::tagged_gui_object ui::create_static_element(gui_static& static_manager, gui_manager& manager, overlapping_region_tag handle, tagged_gui_object parent, ui::overlap_box<B, tag_type, ELEMENT, vertical_extension>& b) {
	const ui::overlapping_region_def& definition = static_manager.ui_definitions.get(handle);
	...
	b.set_self_information(static_manager, manager, new_obj.id, static_cast<int32_t>(definition.spacing), ui::text_aligment_from_overlapping_definition(definition));
