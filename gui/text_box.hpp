...
void ui::display_text<BASE>::update_data(gui_object_tag, gui_manager& m, world_state& w) {
	if constexpr(ui::detail::has_update<BASE, tagged_gui_object, text_data::alignment, ui::text_format&, gui_manager&, world_state&>) {
		ui::clear_children(m, tagged_gui_object{ *associated_object, self });
		BASE::update(tagged_gui_object{ *associated_object, self }, align, format, m, w);
	}
}

template<typename BASE>
template<typename window_type>
void ui::display_text<BASE>::windowed_update(window_type& w, gui_manager& m, world_state& s) {
	if constexpr(ui::detail::has_windowed_update<BASE, window_type&, tagged_gui_object, text_data::alignment, ui::text_format&, gui_manager&, world_state&>) {
		ui::clear_children(m, tagged_gui_object{ *associated_object, self });
		BASE::windowed_update(w, tagged_gui_object{ *associated_object, self }, align, format, m, s);
	}
}
