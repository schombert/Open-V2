...
namespace modifiers {
	...
	void reset_national_modifier(world_state& ws, nations::country_tag this_nation) {
		auto& modifier_values = ws.w.nation_s.nations.get<nation::modifier_values>(this_nation);
		...
		for(auto m : static_range)
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[m];
		...
		for(auto t = timed_range.first; t != timed_range.second; ++t)
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[t->mod];

		if(auto tech_school = ws.w.nation_s.nations.get<nation::tech_school>(this_nation); is_valid_index(tech_school))
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[tech_school];
		if(auto national_value = ws.w.nation_s.nations.get<nation::national_value>(this_nation); is_valid_index(national_value))
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[national_value];

		if(ws.w.nation_s.nations.get<nation::is_civilized>(this_nation) == false)
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.unciv_nation];
		else if(is_great_power(ws, this_nation))
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.great_power];
		else if(ws.w.nation_s.nations.get<nation::overall_rank>(this_nation) <= int16_t(ws.s.modifiers_m.global_defines.colonial_rank))
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.second_power];
		else
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.civ_nation];
		...
		for(int32_t i = int32_t(ws.s.issues_m.issues_container.size()); i--; ) {
			...
			if(is_valid_index(active_option) && is_valid_index(ws.s.issues_m.options[active_option].modifier))
				modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.issues_m.options[active_option].modifier];
		}
		...
		for(int32_t i = int32_t(ws.s.technology_m.technologies_container.size()); i--; ) {
			...
			if(bit_vector_test(active_techs, uint32_t(i)) &&
				...
				modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.technology_m.technologies_container[tag].modifier];
		}
		...
		if(at_war) {
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.war];
		} else {
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.peace];
			if(auto rebel_controlled_provinces  = ws.w.nation_s.nations.get<nation::rebel_controlled_provinces>(this_nation) != 0 && central_province_count != 0)
				modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.total_occupation] * (float(rebel_controlled_provinces) / float(central_province_count));
		}
		
		if(auto infmay = ws.w.nation_s.nations.get<nation::infamy>(this_nation); infmay != 0.0f)
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.badboy] * infmay / ws.s.modifiers_m.global_defines.badboy_limit;
		if(auto blockaded_count = ws.w.nation_s.nations.get<nation::blockaded_count>(this_nation); blockaded_count != 0 && central_province_count != 0)
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.total_blockaded] * (float(blockaded_count) / float(central_province_count));
		if(auto war_exhaustion = ws.w.nation_s.nations.get<nation::war_exhaustion>(this_nation); war_exhaustion != 0.0f)
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.war_exhaustion] * 100.0f * war_exhaustion;
		if(auto plurality = ws.w.nation_s.nations.get<nation::plurality>(this_nation); plurality != 0.0f)
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.plurality] * 100.0f * plurality;
		if(auto disarm_date = ws.w.nation_s.nations.get<nation::disarmed_until>(this_nation);
			...
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.disarming];
		}
		...
		if(total_pop != 0 && literacy != 0)
			modifier_values += ws.s.modifiers_m.national_modifier_definitions[ws.s.modifiers_m.static_modifiers.average_literacy] * (float(literacy) / float(total_pop));
	...
	void detach_province_modifiers(world_state& ws, provinces::province_tag this_province, nations::country_tag nation_from) {
		...
		auto& n_modifier = ws.w.nation_s.nations.get<nation::modifier_values>(nation_from);
		...
		for(auto m : smod) {
			if(auto nm = ws.s.modifiers_m.provincial_modifiers[m].complement; is_valid_index(nm)) {
				...
				n_modifier -= ws.s.modifiers_m.national_modifier_definitions[nm];
			}
		}
	...
	void attach_province_modifiers(world_state& ws, provinces::province_tag this_province, nations::country_tag nation_to) {
		auto smod = get_range(ws.w.province_s.static_modifier_arrays, ws.w.province_s.province_state_container.get<province_state::static_modifiers>(this_province));
		auto& n_modifier = ws.w.nation_s.nations.get<nation::modifier_values>(nation_to);
