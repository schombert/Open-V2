...
public:
	...
	static void rebuild_indexes(modifiers::modifiers_manager& obj) {
		...
		obj.focus_to_pop_types.resize(obj.national_focuses.size());
		obj.focus_to_promotion_amount.resize(obj.national_focuses.size());

		for(auto const& i_nfocus : obj.national_focuses) {
			obj.focus_to_pop_types[i_nfocus.id] = i_nfocus.pop_type;
			obj.focus_to_promotion_amount[i_nfocus.id] = i_nfocus.pop_type_value;
		}
