...
namespace ve {
	...
	struct tagged_vector {
		...
		RELEASE_INLINE tagged_vector(tag_type v) : value(_mm256_set1_epi32(tag_type::zero_is_null_t::value ? int32_t(v.value) : to_index(v))) {}
...
#pragma warning( disable : 4245)
	...
	template<typename U>
	RELEASE_INLINE auto load(__m256i indices, U const* source) -> std::enable_if_t<sizeof(U) == 1 && !std::is_same_v<std::remove_cv_t<U>, bitfield_type>, value_to_vector_type<U>> {
		if constexpr(U(-2) < U(0)) {
