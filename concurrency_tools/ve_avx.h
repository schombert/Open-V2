...
namespace ve {
	...
	struct int_vector {
		...
		__forceinline int_vector() : value_low(_mm_setzero_si128()), value_high(_mm_setzero_si128()) {}
		__forceinline constexpr int_vector(__m128i l, __m128i h) : value_low(l), value_high(h) {}
		__forceinline int_vector(__m256i v) : value_low(_mm256_extractf128_si256(v, 0)), value_high(_mm256_extractf128_si256(v, 1)) {}
		__forceinline int_vector(int32_t v) : value_low(_mm_set1_epi32(v)), value_high(_mm_set1_epi32(v)) {}
		__forceinline int_vector(uint32_t v) : value_low(_mm_set1_epi32(int32_t(v))), value_high(_mm_set1_epi32(int32_t(v))) {}
		__forceinline int_vector(int32_t a, int32_t b, int32_t c, int32_t d, int32_t e, int32_t f, int32_t g, int32_t h) : 
	...
	template<typename U>
	__forceinline auto load(__m256i indices, U const* source) -> std::enable_if_t<sizeof(U) == 4, value_to_vector_type<U>> {
		if constexpr(std::is_same_v<U, float>) {
			return _mm256_setr_ps(
...
#pragma warning( disable : 4245)
	...
	template<typename U>
	__forceinline auto load(__m256i indices, U const* source) -> std::enable_if_t<sizeof(U) == 2, value_to_vector_type<U>> {
		if constexpr(U(-2) < U(0)) {
			const auto casted_source = (int16_t const*)source;
			return _mm256_setr_epi32(
	...
	template<typename U>
	__forceinline auto load(__m256i indices, U const* source) -> std::enable_if_t<sizeof(U) == 1 && !std::is_same_v<std::remove_cv_t<U>, bitfield_type>, value_to_vector_type<U>> {
		if constexpr(U(-2) < U(0)) {
