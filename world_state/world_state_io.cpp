...
void serialization::serializer<current_state::state>::deserialize_object(std::byte const *& input, current_state::state & obj, world_state & ws) {
...
void restore_world_state(world_state& ws) {
	...
	ws.w.nation_s.states.for_each([&ws](nations::state_instance& s) {
		...
		s.administrative_efficiency = nations::calculate_state_administrative_efficiency(ws, s);
	});
	...
	ws.w.population_s.pops.for_each([&ws](population::pop& p) {
		if(is_valid_index(p.associated_army)) {
			...
			a.total_soldiers += ws.w.population_s.pop_demographics.get(p.id, population::total_population_tag);
		}
	});
