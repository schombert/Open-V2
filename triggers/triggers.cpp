...
#pragma clang diagnostic ignored "-Wunused-parameter"
	...
	bool tf_strata_middle(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto pop_type = ((population::pop*)primary_slot)->type;
		if(is_valid_index(pop_type))
	...
	bool tf_strata_poor(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto pop_type = ((population::pop*)primary_slot)->type;
		if(is_valid_index(pop_type))
	...
	bool tf_war_exhaustion_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = population::get_pop_owner(ws, *((population::pop*)primary_slot));
		if(owner)
			return tf_war_exhaustion_nation(tval, ws, owner, nullptr, nullptr, nullptr);
		else
	...
	bool tf_tag_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = population::get_pop_owner(ws, *((population::pop*)primary_slot));
		if(owner)
	...
	bool tf_has_culture_core_province_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_is_greater_power_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = ((provinces::province_state*)primary_slot)->owner;
		if(owner)
			return compare_values(tval[0], nations::is_great_power(ws, *owner), true);
		else
	...
	bool tf_can_nationalize(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_part_of_sphere(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((nations::nation*)primary_slot)->sphere_leader != nullptr, true);
	}
	bool tf_is_sphere_leader_of_tag(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto holder = ws.w.culture_s.national_tags_state[trigger_payload(tval[2]).tag].holder;
		if(holder)
	...
	bool tf_is_sphere_leader_of_from(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((nations::nation*)from_slot)->sphere_leader == primary_slot, true);
	}
	bool tf_is_sphere_leader_of_this_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((nations::nation*)this_slot)->sphere_leader == primary_slot, true);
	}
	bool tf_is_sphere_leader_of_this_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = ((provinces::province_state*)this_slot)->owner;
		if(owner)
	...
	bool tf_is_sphere_leader_of_this_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = ((nations::state_instance*)this_slot)->owner;
		if(owner)
			return compare_values(tval[0], owner->sphere_leader == primary_slot, true);
		else
	...
	bool tf_is_sphere_leader_of_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = population::get_pop_owner(ws, *((population::pop*)this_slot));
		if(owner)
			return compare_values(tval[0], owner->sphere_leader == primary_slot, true);
		else
	...
	bool tf_number_of_states(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_war_score(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		//stub for apparently unused trigger
		return compare_values(tval[0], true, true);
	}
	bool tf_is_releasable_vassal_from(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_is_releasable_vassal_other(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto tag = ((nations::nation*)primary_slot)->tag;
		if(is_valid_index(tag))
			return compare_values(tval[0], !(ws.w.culture_s.national_tags_state[tag].is_not_releasable), true);
		else
	...
	bool tf_has_recent_imigration(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_province_control_days(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_is_disarmed(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_big_producer(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		// stub: used only rarely in ai chances and would be expensive to test
		return compare_values(tval[0], false, true);
	}
	bool tf_someone_can_form_union_tag_from(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		// stub: apparently unused
		return compare_values(tval[0], false, true);
	}
	bool tf_someone_can_form_union_tag_other(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		// stub: apparently unused
		return compare_values(tval[0], false, true);
	}
	bool tf_social_movement_strength(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((nations::nation*)primary_slot)->social_movement_support, read_float_from_payload(tval + 2));
	}
	bool tf_political_movement_strength(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((nations::nation*)primary_slot)->political_movement_support, read_float_from_payload(tval + 2));
	}
	bool tf_can_build_factory_in_capital_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		// stub: virtually unused
		return compare_values(tval[0], true, true);
	}
	bool tf_social_movement(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_political_movement(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto mt = ((population::pop*)primary_slot)->movement;
		if(is_valid_index(mt))
	...
	bool tf_has_cultural_sphere(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto prim_culture = ((nations::nation*)primary_slot)->primary_culture;
	...
	bool tf_world_wars_enabled(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ws.w.world_wars_enabled, true);
	}
	bool tf_has_pop_culture_pop_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((population::pop*)primary_slot)->culture == ((population::pop*)this_slot)->culture, true);
	}
	bool tf_has_pop_culture_state_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto culture = ((population::pop*)this_slot)->culture;
		auto id = ((nations::state_instance*)primary_slot)->id;
	...
	bool tf_has_pop_culture_province_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto culture = ((population::pop*)this_slot)->culture;
		auto id = ((provinces::province_state*)primary_slot)->id;
	...
	bool tf_has_pop_culture_nation_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto culture = ((population::pop*)this_slot)->culture;
		auto id = ((nations::nation*)primary_slot)->id;
	...
	bool tf_has_pop_culture_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((population::pop*)primary_slot)->culture == trigger_payload(tval[2]).culture, true);
	}
	bool tf_has_pop_culture_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::state_instance*)primary_slot)->id;
		if(ws.w.nation_s.states.is_valid_index(id))
	...
	bool tf_has_pop_culture_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((provinces::province_state*)primary_slot)->id;
	...
	bool tf_has_pop_culture_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::nation*)primary_slot)->id;
		if(ws.w.nation_s.nations.is_valid_index(id))
	...
	bool tf_has_pop_religion_pop_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((population::pop*)primary_slot)->religion == ((population::pop*)this_slot)->religion, true);
	}
	bool tf_has_pop_religion_state_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_has_pop_religion_province_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto religion = ((population::pop*)this_slot)->religion;
		auto id = ((provinces::province_state*)primary_slot)->id;
	...
	bool tf_has_pop_religion_nation_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto religion = ((population::pop*)this_slot)->religion;
		auto id = ((nations::nation*)primary_slot)->id;
	...
	bool tf_has_pop_religion_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((population::pop*)primary_slot)->religion == trigger_payload(tval[2]).small.values.religion, true);
	}
	bool tf_has_pop_religion_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::state_instance*)primary_slot)->id;
		if(ws.w.nation_s.states.is_valid_index(id))
	...
	bool tf_has_pop_religion_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((provinces::province_state*)primary_slot)->id;
	...
	bool tf_has_pop_religion_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::nation*)primary_slot)->id;
		if(ws.w.nation_s.nations.is_valid_index(id))
	...
	bool tf_life_needs(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((population::pop*)primary_slot)->needs_satisfaction, read_float_from_payload(tval + 2));
	}
	bool tf_everyday_needs(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((population::pop*)primary_slot)->needs_satisfaction, read_float_from_payload(tval + 2) + 1.0f);
	}
	bool tf_luxury_needs(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((population::pop*)primary_slot)->needs_satisfaction, read_float_from_payload(tval + 2) + 2.0f);
	}
	bool tf_consciousness_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_consciousness_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((provinces::province_state*)primary_slot)->id;
		auto total_pop = ws.w.province_s.province_demographics.get(id, population::total_population_tag);
	...
	bool tf_consciousness_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::state_instance*)primary_slot)->id;
		if(ws.w.nation_s.states.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.state_demographics.get(id, population::total_population_tag);
	...
	bool tf_consciousness_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::nation*)primary_slot)->id;
		if(ws.w.nation_s.nations.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.nation_demographics.get(id, population::total_population_tag);
	...
	bool tf_literacy_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_literacy_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((provinces::province_state*)primary_slot)->id;
		auto total_pop = ws.w.province_s.province_demographics.get(id, population::total_population_tag);
	...
	bool tf_literacy_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::state_instance*)primary_slot)->id;
		if(ws.w.nation_s.states.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.state_demographics.get(id, population::total_population_tag);
	...
	bool tf_literacy_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::nation*)primary_slot)->id;
		if(ws.w.nation_s.nations.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.nation_demographics.get(id, population::total_population_tag);
	...
	bool tf_militancy_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], float(((population::pop*)primary_slot)->militancy) * 10.0f / float(std::numeric_limits<uint16_t>::max()), read_float_from_payload(tval + 2));
	}
	bool tf_militancy_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((provinces::province_state*)primary_slot)->id;
		auto total_pop = ws.w.province_s.province_demographics.get(id, population::total_population_tag);
		auto con = ws.w.province_s.province_demographics.get(id, population::militancy_demo_tag(ws));
		return compare_values(tval[0], total_pop != 0 ? (float(con) * 10.0f / float(total_pop)) : 0.0f, read_float_from_payload(tval + 2));
	}
	bool tf_militancy_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::state_instance*)primary_slot)->id;
		if(ws.w.nation_s.states.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.state_demographics.get(id, population::total_population_tag);
	...
	bool tf_militancy_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::nation*)primary_slot)->id;
		if(ws.w.nation_s.nations.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.nation_demographics.get(id, population::total_population_tag);
	...
	bool tf_trade_goods_in_state_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_trade_goods_in_state_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto si = ((provinces::province_state*)primary_slot)->state_instance;
		if(si)
	...
	bool tf_has_flashpoint(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], is_valid_index(((nations::state_instance*)primary_slot)->crisis_tag), true);
	}
	bool tf_flashpoint_tension(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((nations::state_instance*)primary_slot)->current_tension, read_float_from_payload(tval + 2));
	}
	bool tf_crisis_exist(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ws.w.current_crisis_type != current_state::crisis_type::none, true);
	}
	bool tf_is_liberation_crisis(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ws.w.current_crisis_type == current_state::crisis_type::liberation, true);
	}
	bool tf_is_claim_crisis(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ws.w.current_crisis_type == current_state::crisis_type::claim, true);
	}
	bool tf_crisis_temperature(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ws.w.crisis_temperature, read_float_from_payload(tval + 2));
	}
	bool tf_involved_in_crisis_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::nation*)primary_slot)->id;
		return compare_values(tval[0],
	...
	bool tf_involved_in_crisis_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = population::get_pop_owner(ws, *((population::pop*)primary_slot));
		if(owner)
	...
	bool tf_rich_strata_life_needs_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto location = ((population::pop*)primary_slot)->location;
		if(is_valid_index(location)) {
	...
	bool tf_rich_strata_everyday_needs_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto location = ((population::pop*)primary_slot)->location;
		if(is_valid_index(location)) {
			auto si = ws.w.province_s.province_state_container[location].state_instance;
			if(si)
	...
	bool tf_rich_strata_luxury_needs_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto location = ((population::pop*)primary_slot)->location;
		if(is_valid_index(location)) {
			auto si = ws.w.province_s.province_state_container[location].state_instance;
			if(si)
	...
	bool tf_middle_strata_life_needs_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto location = ((population::pop*)primary_slot)->location;
		if(is_valid_index(location)) {
			auto si = ws.w.province_s.province_state_container[location].state_instance;
			if(si)
	...
	bool tf_middle_strata_everyday_needs_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto location = ((population::pop*)primary_slot)->location;
		if(is_valid_index(location)) {
			auto si = ws.w.province_s.province_state_container[location].state_instance;
			if(si)
	...
	bool tf_middle_strata_luxury_needs_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto location = ((population::pop*)primary_slot)->location;
		if(is_valid_index(location)) {
			auto si = ws.w.province_s.province_state_container[location].state_instance;
			if(si)
	...
	bool tf_poor_strata_life_needs_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto location = ((population::pop*)primary_slot)->location;
		if(is_valid_index(location)) {
			auto si = ws.w.province_s.province_state_container[location].state_instance;
			if(si)
	...
	bool tf_poor_strata_everyday_needs_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto location = ((population::pop*)primary_slot)->location;
		if(is_valid_index(location)) {
			auto si = ws.w.province_s.province_state_container[location].state_instance;
			if(si)
	...
	bool tf_poor_strata_luxury_needs_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto location = ((population::pop*)primary_slot)->location;
		if(is_valid_index(location)) {
			auto si = ws.w.province_s.province_state_container[location].state_instance;
			if(si)
	...
	bool tf_diplomatic_influence_tag(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_diplomatic_influence_this_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_diplomatic_influence_this_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = ((provinces::province_state*)this_slot)->owner;
		if(owner)
	...
	bool tf_diplomatic_influence_from_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_diplomatic_influence_from_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_pop_unemployment_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_pop_unemployment_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto type = trigger_payload(tval[4]).small.values.pop_type;
		auto id = ((nations::state_instance*)primary_slot)->id;
		if(ws.w.nation_s.states.is_valid_index(id)) {
	...
	bool tf_pop_unemployment_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto type = trigger_payload(tval[4]).small.values.pop_type;
		auto id = ((provinces::province_state*)primary_slot)->id;
	...
	bool tf_pop_unemployment_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((population::pop*)primary_slot)->id;
		if(ws.w.population_s.pops.is_valid_index(id)) {
	...
	bool tf_pop_unemployment_nation_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_pop_unemployment_state_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto type = ((population::pop*)this_slot)->type;
		auto id = ((nations::state_instance*)primary_slot)->id;
		if(ws.w.nation_s.states.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.state_demographics.get(id, population::to_demo_tag(ws, type));
	...
	bool tf_pop_unemployment_province_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto type = ((population::pop*)this_slot)->type;
		auto id = ((provinces::province_state*)primary_slot)->id;
		auto total_pop = ws.w.province_s.province_demographics.get(id, population::to_demo_tag(ws, type));
		if(total_pop != 0)
	...
	bool tf_relation_tag(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto holder = ws.w.culture_s.national_tags_state[trigger_payload(tval[3]).tag].holder;
		if(holder)
	...
	bool tf_relation_this_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_relation_this_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = ((provinces::province_state*)this_slot)->owner;
		if(owner)
	...
	bool tf_relation_from_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_relation_from_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = ((provinces::province_state*)from_slot)->owner;
		if(owner)
			return tf_relation_this_nation(tval, ws, primary_slot, owner, nullptr, nullptr);
		else
	...
	bool tf_check_variable(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::nation*)primary_slot)->id;
		if(ws.w.nation_s.nations.is_valid_index(id))
	...
	bool tf_upper_house(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::nation*)primary_slot)->id;
		if(ws.w.nation_s.nations.is_valid_index(id))
	...
	bool tf_unemployment_by_type_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return tf_pop_unemployment_nation(tval, ws, primary_slot, nullptr, nullptr, nullptr);
	}
	bool tf_unemployment_by_type_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return tf_pop_unemployment_state(tval, ws, primary_slot, nullptr, nullptr, nullptr);
	}
	bool tf_unemployment_by_type_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return tf_pop_unemployment_province(tval, ws, primary_slot, nullptr, nullptr, nullptr);
	}
	bool tf_unemployment_by_type_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto location = ((population::pop*)primary_slot)->location;
		if(is_valid_index(location)) {
			auto si = ws.w.province_s.province_state_container[location].state_instance;
			if(si)
	...
	bool tf_party_loyalty_nation_province_id(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_party_loyalty_from_nation_province_id(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ws.w.province_s.party_loyalty.get(provinces::province_tag(tval[2]), trigger_payload(tval[5]).small.values.ideology), read_float_from_payload(tval + 3));
	}
	bool tf_party_loyalty_province_province_id(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ws.w.province_s.party_loyalty.get(provinces::province_tag(tval[2]), trigger_payload(tval[5]).small.values.ideology), read_float_from_payload(tval + 3));
	}
	bool tf_party_loyalty_from_province_province_id(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ws.w.province_s.party_loyalty.get(provinces::province_tag(tval[2]), trigger_payload(tval[5]).small.values.ideology), read_float_from_payload(tval + 3));
	}
	bool tf_party_loyalty_nation_from_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_party_loyalty_generic(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_party_loyalty_from_nation_scope_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ws.w.province_s.party_loyalty.get(((provinces::province_state*)primary_slot)->id, trigger_payload(tval[4]).small.values.ideology), read_float_from_payload(tval + 2));
	}
	bool tf_party_loyalty_from_province_scope_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ws.w.province_s.party_loyalty.get(((provinces::province_state*)primary_slot)->id, trigger_payload(tval[4]).small.values.ideology), read_float_from_payload(tval + 2));
	}
	...
	bool tf_can_build_in_province_fort_no_limit_from_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0],
	...
	bool tf_can_build_in_province_fort_yes_limit_from_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0],
			technologies::tech_attribute_type(((provinces::province_state*)primary_slot)->fort_level) <
				((nations::nation*)from_slot)->tech_attributes[technologies::tech_offset::max_fort],
			true);
	}
	bool tf_can_build_in_province_fort_no_limit_this_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0],
			technologies::tech_attribute_type(((provinces::province_state*)primary_slot)->fort_level) <
				((nations::nation*)this_slot)->tech_attributes[technologies::tech_offset::max_fort],
			true);
	}
	bool tf_can_build_in_province_fort_yes_limit_this_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0],
			technologies::tech_attribute_type(((provinces::province_state*)primary_slot)->fort_level) <
				((nations::nation*)this_slot)->tech_attributes[technologies::tech_offset::max_fort],
			true);
	}
	bool tf_can_build_in_province_naval_base_no_limit_from_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_can_build_in_province_naval_base_yes_limit_from_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		if(((provinces::province_state*)primary_slot)->naval_base_level != 0ui8) {
			return compare_values(tval[0],
				technologies::tech_attribute_type(((provinces::province_state*)primary_slot)->naval_base_level) <
				((nations::nation*)from_slot)->tech_attributes[technologies::tech_offset::max_naval_base],
	...
	bool tf_can_build_in_province_naval_base_no_limit_this_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		if(((provinces::province_state*)primary_slot)->naval_base_level != 0ui8) {
			return compare_values(tval[0],
				technologies::tech_attribute_type(((provinces::province_state*)primary_slot)->naval_base_level) <
	...
	bool tf_can_build_in_province_naval_base_yes_limit_this_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		if(((provinces::province_state*)primary_slot)->naval_base_level != 0ui8) {
			return compare_values(tval[0],
				technologies::tech_attribute_type(((provinces::province_state*)primary_slot)->naval_base_level) <
				((nations::nation*)this_slot)->tech_attributes[technologies::tech_offset::max_naval_base],
	...
	bool tf_can_build_railway_in_capital_yes_whole_state_yes_limit(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		// stub: virtually unused
		return compare_values(tval[0], true, true);
	}
	bool tf_can_build_railway_in_capital_yes_whole_state_no_limit(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		// stub: virtually unused
		return compare_values(tval[0], true, true);
	}
	bool tf_can_build_railway_in_capital_no_whole_state_yes_limit(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		// stub: virtually unused
		return compare_values(tval[0], true, true);
	}
	bool tf_can_build_railway_in_capital_no_whole_state_no_limit(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		// stub: virtually unused
		return compare_values(tval[0], true, true);
	}
	bool tf_can_build_fort_in_capital_yes_whole_state_yes_limit(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		// stub: virtually unused
		return compare_values(tval[0], true, true);
	}
	bool tf_can_build_fort_in_capital_yes_whole_state_no_limit(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		// stub: virtually unused
		return compare_values(tval[0], true, true);
	}
	bool tf_can_build_fort_in_capital_no_whole_state_yes_limit(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		// stub: virtually unused
		return compare_values(tval[0], true, true);
	}
	bool tf_can_build_fort_in_capital_no_whole_state_no_limit(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		// stub: virtually unused
		return compare_values(tval[0], true, true);
	}
	bool tf_work_available_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_work_available_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto count_workers = tval[1] - 1;
		auto id = ((nations::state_instance*)primary_slot)->id;
	...
	bool tf_work_available_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto count_workers = tval[1] - 1;
		auto id = ((provinces::province_state*)primary_slot)->id;
	...
	bool tf_variable_ideology_name_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::nation*)primary_slot)->id;
		if(ws.w.nation_s.nations.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.nation_demographics.get(id, population::total_population_tag);
			if(total_pop != 0)
	...
	bool tf_variable_ideology_name_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::state_instance*)primary_slot)->id;
		if(ws.w.nation_s.states.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.state_demographics.get(id, population::total_population_tag);
			if(total_pop != 0)
	...
	bool tf_variable_ideology_name_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((provinces::province_state*)primary_slot)->id;
	...
	bool tf_variable_ideology_name_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((population::pop*)primary_slot)->id;
		if(ws.w.population_s.pops.is_valid_index(id)) {
	...
	bool tf_variable_issue_name_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::nation*)primary_slot)->id;
		if(ws.w.nation_s.nations.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.nation_demographics.get(id, population::total_population_tag);
			if(total_pop != 0)
	...
	bool tf_variable_issue_name_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::state_instance*)primary_slot)->id;
		if(ws.w.nation_s.states.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.state_demographics.get(id, population::total_population_tag);
			if(total_pop != 0)
	...
	bool tf_variable_issue_name_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((provinces::province_state*)primary_slot)->id;
	...
	bool tf_variable_issue_name_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((population::pop*)primary_slot)->id;
		if(ws.w.population_s.pops.is_valid_index(id)) {
			auto total_pop = ws.w.population_s.pop_demographics.get(id, population::total_population_tag);
			if(total_pop != 0)
	...
	bool tf_variable_issue_group_name_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_variable_issue_group_name_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = ((nations::state_instance*)primary_slot)->owner;
		if(owner)
	...
	bool tf_variable_issue_group_name_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = ((provinces::province_state*)primary_slot)->owner;
		if(owner)
			return tf_variable_issue_group_name_nation(tval, ws, owner, nullptr, nullptr, nullptr);
		else
	...
	bool tf_variable_issue_group_name_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = population::get_pop_owner(ws, *((population::pop*)primary_slot));
		if(owner)
			return tf_variable_issue_group_name_nation(tval, ws, owner, nullptr, nullptr, nullptr);
		else
	...
	bool tf_variable_pop_type_name_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::nation*)primary_slot)->id;
		if(ws.w.nation_s.nations.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.nation_demographics.get(id, population::total_population_tag);
			if(total_pop != 0)
	...
	bool tf_variable_pop_type_name_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::state_instance*)primary_slot)->id;
		if(ws.w.nation_s.states.is_valid_index(id)) {
			auto total_pop = ws.w.nation_s.state_demographics.get(id, population::total_population_tag);
			if(total_pop != 0)
	...
	bool tf_variable_pop_type_name_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((provinces::province_state*)primary_slot)->id;
	...
	bool tf_variable_pop_type_name_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_variable_good_name(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto id = ((nations::nation*)primary_slot)->id;
		if(ws.w.nation_s.nations.is_valid_index(id))
	...
	bool tf_religion_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_religion_nation_reb(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		return compare_values(tval[0], ((nations::nation*)primary_slot)->national_religion == rebel_slot->religion, true);
	}
	bool tf_religion_nation_from_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_religion_nation_this_nation(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
	...
	bool tf_religion_nation_this_state(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = ((nations::state_instance*)this_slot)->owner;
		if(owner)
	...
	bool tf_religion_nation_this_province(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = ((provinces::province_state*)this_slot)->owner;
		if(owner)
			return tf_religion_nation_this_nation(tval, ws, owner, nullptr, nullptr, nullptr);
		else
	...
	bool tf_religion_nation_this_pop(uint16_t const* tval, world_state& ws, void* primary_slot, void* this_slot, void* from_slot, population::rebel_faction* rebel_slot) {
		auto owner = population::get_pop_owner(ws, *((population::pop*)this_slot));
		if(owner)
			return tf_religion_nation_this_nation(tval, ws, owner, nullptr, nullptr, nullptr);
		else
